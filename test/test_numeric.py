...
class TestNumericColumn(unittest.TestCase):
    def setUp(self):
        # Load data from a URL for testing purposes
    ...
    def test_get_unique(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 57  # Adjust according to your expected data
        result = ncol.get_unique()
        self.assertEqual(result, expected)

    def test_get_missing(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 2  # Adjust if different
        result = ncol.get_missing()
        self.assertEqual(result, expected)

    def test_get_zeros(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 0  # Adjust if different
        result = ncol.get_zeros()
        self.assertEqual(result, expected)

    def test_get_negatives(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 1  # Adjust if different
        result = ncol.get_negatives()
        self.assertEqual(result, expected)

    def test_get_mean(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 36.840089285714285  # Adjust if different
        result = ncol.get_mean()
        self.assertAlmostEqual(result, expected, places=5)

    def test_get_std(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 10.887035414985837  # Adjust if different
        result = ncol.get_std()
        self.assertAlmostEqual(result, expected, places=5)

    def test_get_min(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = -14.271  # Adjust if different
        result = ncol.get_min()
        self.assertEqual(result, expected)

    def test_get_max(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 61.3707  # Adjust if different
        result = ncol.get_max()
        self.assertEqual(result, expected)

    def test_get_median(self):
        ncol = NumericColumn(col_name='Lat', serie=self.data['Lat'])
        expected = 39.06185  # Adjust if different
    ...
    def test_get_frequent(self):
        ...
        expected = pd.DataFrame({
            'value': [42.7560, 44.2685, 15.0979, 40.3888, 35.5653, 44.5720, 40.5908,
